...
window.runEMSOps = function(userMes) {
  ...
  // Global counter for consecutive authentication failures
  let _consecutiveAuthFailures = 0;
  const MAX_AUTH_FAILURES = 3; // Number of consecutive failures before triggering a full reload

  // Function to handle critical session issues
  function handleCriticalSessionError(errorMessage) {
    console.error('[Dashboard] Erro crítico de sessão:', errorMessage);
    showToast(errorMessage, 'error');
    stopPolling(); // Stop all polling to prevent further errors

    if (window.opener && !window.opener.closed) {
      // Signal to the main window to reload the dashboard
      // This assumes the opener (popup.js or content.js) has a mechanism to handle this.
      window.opener.postMessage({ type: 'EMS_DASHBOARD_SESSION_EXPIRED' }, '*');
      // As a fallback, also try to reload this window after a short delay
      setTimeout(() => location.reload(), 3000);
    } else {
      // If no opener, reload this window directly
      setTimeout(() => location.reload(), 1000);
    }
    throw new Error(errorMessage); // Stop current execution flow
  }
  ...
  const render = (ativos, postMortem, slaA, slaP, m, resolvedToday, aggData, eventTasks=[]) => {
    ...
    const escAttr = v => String(v == null ? '' : v).replace(/[&<>"']/g, ch => ({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' }[ch]));
    ...
    // Seed the resolved today "seen" set to prevent duplicate counts during real-time updates
    const todayStrLocal = new Date().toLocaleDateString('en-CA'); // YYYY-MM-DD
    if (window._resolvedTodaySeenDate !== todayStrLocal) {
      window._resolvedTodaySeenDate = todayStrLocal;
      window._resolvedTodaySeen.clear();
    }
    (resolvedToday || []).forEach(c => {
      const sid = c.sys_id?.value || c.sys_id;
      if (sid) window._resolvedTodaySeen.add(sid);
    });
...
async function snFetch(url, opts = {}) {
  ...
  try {
    ...
    console.error('[Dashboard] snFetch: Erro de rede ou fetch:', e);
    _consecutiveAuthFailures++; // Treat network errors as potential session issues
    if (_consecutiveAuthFailures >= MAX_AUTH_FAILURES) {
      handleCriticalSessionError('Falha de conexão persistente. Recarregando dashboard...');
    }
  ...
  if ((r.status === 401 || r.status === 403) && window.opener && !window.opener.closed) {
    try {
      ...
      if (nt && nt !== _TOK) {
        console.log('[Dashboard] snFetch: Sessão renovada via ServiceNow.');
        _TOK = nt;
      ...
      console.error('[Dashboard] snFetch: Erro ao tentar renovar token via opener:', e);
    }
  }

  if (r.status === 401 || r.status === 403) {
    _consecutiveAuthFailures++;
    console.warn('[Dashboard] snFetch: Falha de autenticação (' + _consecutiveAuthFailures + '/' + MAX_AUTH_FAILURES + ').');
    if (_consecutiveAuthFailures >= MAX_AUTH_FAILURES) {
      handleCriticalSessionError('Sessão expirada. Recarregando dashboard...');
    }
  } else {
    _consecutiveAuthFailures = 0; // Reset counter on success
  }
  return r;
}
...
function renderFilterChips(){
  ...
  filaGroup.innerHTML=Array.from(filaSel.options).map(o=>{
    const count=(queueCounts[o.value] != null ? queueCounts[o.value] : (o.value===filaSel.value?currentBoardCount:0));
  ...
  if(elCS){
    ...
    .then(d=>{
      ...
      rows.forEach(row=>{
        ...
        const val=Number((row.groupby_fields?.[1]?.value != null ? row.groupby_fields?.[1]?.value : row.groupby_fields?.[0]?.value));
        total+=cnt;
...
function populateAccountProducts(listEl, accountId, accountName, ciId, ciName){
  ...
  const getVal = (obj, ...keys) => {
    for (const k of keys){
      const node = obj?.[k];
      const v = node?.display_value != null ? node.display_value : (node?.value != null ? node.value : node);
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  (function() {
    ...
    function shouldCountResolvedTodayOnce(sysId, resolvedDate) {
      ...
      if (!sysId || !resolvedDate || !resolvedDate.includes(today)) return false;
      if (window._resolvedTodaySeen.has(sysId)) return false;
      window._resolvedTodaySeen.add(sysId);
      return true;
    }
    ...
    function updateResolvedTodayUI(data) {
      ...
      const resolvedDate = data?.resolved_at?.value || data?.closed_at?.value || '';
    ...
    async function fetchDeltas() {
      ...
      const isResolvedToday = c => {
        const st = String(c?.state?.value || '');
        if (st !== '6' && st !== '7' && st !== '3') return false;
        const resDate = (c.resolved_at?.value || c.closed_at?.value || '');
        return resDate.includes(getTodayDateLocal());
      };
      ...
      const evaluateCardVisibility = (card, data) => {
        ...
        const isBl = bWrap.id === 'board-wrap-backlog-tab';
        const aId = isBl ? (currentBacklogAnalyst || document.getElementById('backlog-analyst-sel')?.value || '') : (document.getElementById('analyst-sel')?.value || '');
        const mId = isBl ? '' : (document.getElementById('manager-sel')?.value || '');
        const fK = isBl ? currentBacklogFila : currentFila;
        const g = window._GID_MAP?.[fK] || '';

        // Use data for assigned_to if available, otherwise fallback to dataset
        const cAssId = data?.assigned_to?.value || card.dataset.assignedid || '';
    ...
    function insertNewCaseCard(data, visibilityCallback) {
      ...
      const nowMs = Date.now();
      const openedAtMs = new Date(data.opened_at?.value || 0).getTime();
      const isBacklogData = (nowMs - openedAtMs) >= (20 * 86400000);
      
      // 1. Kanban Board (Ativos)
      if (!isBacklogData && queueMatchesFila(gid, currentFila)) {
      ...
      // 2. Backlog Board
      if (isBacklogData && queueMatchesFila(gid, currentBacklogFila)) {
...
<div id="account-products-overlay" style="display:none;position:fixed;inset:0;z-index:700;background:rgba(15,23,42,.55);align-items:center;justify-content:center;">
  ...
  window._emsOpsRender = (tok, m2, targetWin, uid2) => {
    ...
    const todayStart2 = new Date().toLocaleDateString('en-CA') + ' 00:00:00';
    const todayEnd2   = new Date().toLocaleDateString('en-CA') + ' 23:59:59';
    const qRT2=`assignment_groupIN${G_IDS}^stateIN6,7,3^resolved_at>=${todayStart2}^resolved_at<=${todayEnd2}`;
