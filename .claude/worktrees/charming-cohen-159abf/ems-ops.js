...
function openCaseModal(sysId, number, cardEl, recordTable='sn_customerservice_case') {
  ...
  fetch(_BASE+'/api/now/table/sn_customerservice_case/'+sysId+'?sysparm_fields=number,account,cmdb_ci&sysparm_display_value=all',{headers:h})
    ...
    .then(data=>{
      ...
      if (listEl)  populateAccountProducts(listEl, accId, accName, ciId, ciName, { isEventTask, cardEl, number });
    })
    .catch(()=>{
...
function populateAccountProducts(listEl, accountId, accountName, ciId, ciName, ctx){
  const isEventTask = !!(ctx && ctx.isEventTask);
  const cardEl = (ctx && ctx.cardEl) || null;
